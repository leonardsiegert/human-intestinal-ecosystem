...
var margin = { top: 50, right: 0, bottom: 100, left: 300 },
          ...
          tests = ["underweight","lean","overweight","obese","severeobese","morbidobese"];
          
let svg = d3.select("#BMI")
...
d3.csv(dataset).then(function (data){
	var colorScale = d3.scaleQuantile()        // Taking the value by quantile, the number of elements in each area can be equal
      .domain([700, 180000])  // domain([min,max])
      .range(colors); 
  ...
  var timeLabels = svg.selectAll(".testLabel")
   ...
   for (var i = 0; i < 12; i++){
    array_data[i*6] = data[i].underweight;
    array_data[i*6+1] = data[i].lean;
    array_data[i*6+2] = data[i].overweight;
    array_data[i*6+3] = data[i].obese;
    array_data[i*6+4] = data[i].severeobese;
    array_data[i*6+5] = data[i].morbidobese;
    }
  ...
  var legend = svg.selectAll(".legend")
      .data([700].concat(colorScale.quantiles()), function(d) { return d; })
      .enter().append("g")
      .attr("class", "legend");
