...
var margin = { top: 50, right: 0, bottom: 100, left: 300 },
          ...
          tests = ["CentralEurope","EasternEurope","Scandinavia","SouthEurope","UKIE","US"];
          
let svg = d3.select("#nationality")
...
d3.csv(dataset).then(function (data){
	var colorScale = d3.scaleQuantile()        // Taking the value by quantile, the number of elements in each area can be equal
      .domain([370, 120000])  // domain([min,max])
      .range(colors); 
  ...
  var timeLabels = svg.selectAll(".testLabel")
   ...
   for (var i = 0; i < 14; i++){
    array_data[i*6] = data[i].CentralEurope;
    array_data[i*6+1] = data[i].EasternEurope;
    array_data[i*6+2] = data[i].Scandinavia;
    array_data[i*6+3] = data[i].SouthEurope;
    array_data[i*6+4] = data[i].UKIE;
    array_data[i*6+5] = data[i].US;
    }
  ...
  var heatMap = svg.selectAll(".score")
      ...
      .enter()       
      .append("rect")
      .attr("x", function(d, i){ return (i % 6)*gridSize;})
      .attr("y", function(d, i){ return parseInt(i / 6)*gridSize;})
      .attr("rx", 6)
      .attr("ry", 6)
  ...
  var legend = svg.selectAll(".legend")
      .data([370].concat(colorScale.quantiles()), function(d) { return d; })
      .enter().append("g")
      .attr("class", "legend");
  ...
  legend.append("text")
    ...
    .attr("x", 460+gridSize/3)
    .attr("y", function(d, i) { return 40+gridSize * i; });
});
