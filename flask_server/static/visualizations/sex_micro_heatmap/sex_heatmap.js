...
d3.csv(dataset).then(function (data){
	var colorScale = d3.scaleQuantile()        // Taking the value by quantile, the number of elements in each area can be equal
      .domain([12000, 120000])  // domain([min,max])
      .range(colors); 
  ...
  var timeLabels = svg.selectAll(".testLabel")
   ...
   for (var i = 0; i < 9; i++){
    array_data[i*2] = data[i].female;
    array_data[i*2+1] = data[i].male;
    }
  ...
  var heatMap = svg.selectAll(".score")
      ...
      .enter()       
      .append("rect")
      .attr("x", function(d, i){ return (i % 2)*gridSize;})
      .attr("y", function(d, i){ return parseInt(i / 2)*gridSize;})
      .attr("rx", 6)
      .attr("ry", 6)
  ...
  var legend = svg.selectAll(".legend")
      .data([12000].concat(colorScale.quantiles()), function(d) { return d; })
      .enter().append("g")
      .attr("class", "legend");
